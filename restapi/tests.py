...
class CitizenTestCase(TestCase):
    def setUp(self):
        ...
        self.profile_user = Profile(user=self.user, national_id=self.profile_national_id)
...
class ViewCitizenTestCase(TestCase):
    def setUp(self):
        ...
        self.response = self.client.post(
            reverse("citizen_create"), self.citizen_data, format="json"
        )
    ...
    def test_api_can_get_a_citizen(self):
        ...
        response = self.client.get(
            reverse("citizen_record", kwargs={"user__username": profile.user.username}),
            format="json",
        )
...
class ViewVoteTestCase(TestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User(id=1, username="test", email="test@test.com", password="test")
        self.user.save()
    ...
    def test_api_can_vote_review(self):
        ...
        response = self.client.post(
            reverse("reviewing_vote_proposal"), self.vote_data, format="json"
        )
