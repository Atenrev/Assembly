...
def make_vote(user, phase, proposal, option, user_pw):
    ...
    try:
        ...
        uservote = UserProposalPhaseVote(
            phase=phase, proposal=proposal, user=user)
        anonvote = ProposalPhaseVote(
            phase=phase, proposal=proposal, user_pw=user_pw,
            option=option, hash=digest, identifier=uuid.uuid4().hex,
